...
import { enqueueSell } from "./sellqueue.js";
import { markPriceCheck } from "./health.js";
...
async function evaluatePosition(
  conn: Connection, kp: Keypair, p: Position,
): Promise<void> {
  ...
  if (config.stopLossPct > 0 && pnlPct <= -config.stopLossPct) {
    ...
    config.takeProfitPct > 0 && pnlPct >= config.takeProfitPct &&
    config.trailingStopPct > 0 && drawdownFromPeak >= config.trailingStopPct
  ) {
    reason = `trailing take-profit +${(pnlPct * 100).toFixed(1)}% (dropped ${(drawdownFromPeak * 100).toFixed(1)}% from peak)`;
  } else if (
    config.takeProfitPct > 0 && config.trailingStopPct <= 0 && pnlPct >= config.takeProfitPct
  ) {
    reason = `take-profit +${(pnlPct * 100).toFixed(1)}%`;
  } else if (config.maxPositionAgeHours > 0 && ageHours >= config.maxPositionAgeHours) {
  ...
  try {
    ...
    recordTrade({
      ...
      txSig: res.signature,
      reason: `${reason} · ${res.route}`,
    });
    incr("auto_exits_total");
    notify(
      `🛑 auto-exit *${p.mint.slice(0, 6)}…* (${reason})\nspent ${fmtSol(p.solSpentLamports)} → recv ${fmtSol(res.solLamports)} SOL`,
    );
  } catch (err) {
    log.error(
      { err: (err as Error).message, mint: p.mint, reason },
      "auto-exit sell failed — enqueuing for retry",
    );
    incr("auto_exit_deferred_total");
    enqueueSell({
      mint: p.mint, leader: p.leader,
      amountRaw: amount, reason,
    });
  }
}
